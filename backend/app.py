...
# Inicializar extensiones SIN la app primero
db = SQLAlchemy()
bcrypt = Bcrypt()
jwt = JWTManager()

# Luego inicializar las extensiones con la app
db.init_app(app)
bcrypt.init_app(app)
jwt.init_app(app)
...
# IMPORTANTE: Importa los modelos DESPUÉS de crear e inicializar 'db'
from models import *
...
def get_corridas():
    ...
    try:
        ...
        # Usar el contexto de aplicación para las consultas
        with app.app_context():
...
def get_asientos():
    ...
    try:
        with app.app_context():
           ...
           # 3. Buscar todos los asientos ocupados (RESERVADOS y BLOQUEADOS)
...
def crear_reserva():
    ...
    try:
        with app.app_context():
            ...
            # 9. Confirmar los cambios en la base de datos
            db.session.commit()
...
def validar_ticket():
    ...
    try:
        with app.app_context():
            ...
            # 4. Si existe y es válido, buscamos los detalles
...
def bloquear_asientos():
    ...
    try:
        with app.app_context():
            ...
            # 2. Chequeo de bloqueos actuales
            ...
            # 3. Si están libres, crear bloqueo
