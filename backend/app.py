...
# Declaramos las extensiones como globales, pero vacías
db = SQLAlchemy()
bcrypt = Bcrypt()
jwt = JWTManager()
# (Importamos los modelos DESPUÉS de definir 'db')
...
def create_app():
    ...
    Railway (Gunicorn) llamará a esta función.
    """
    app = Flask(__name__)

    # --- 2. Configuración desde Variables de Entorno (¡CLAVE!) ---
    ...
    app.config['JWT_SECRET_KEY'] = os.environ.get('JWT_SECRET_KEY', 'default-secret-key-para-local')
    app.config['STRIPE_SECRET_KEY'] = os.environ.get('STRIPE_SECRET_KEY')
    app.config['STRIPE_WEBHOOK_SECRET'] = os.environ.get('STRIPE_WEBHOOK_SECRET')
    
    app.config['JWT_ACCESS_TOKEN_EXPIRES'] = timedelta(hours=1)
    
    # Asignamos la llave a Stripe
    ...
    # --- 4. Configurar CORS ---
    # Permitimos cualquier origen (o puedes especificar tu URL de Vercel)
    CORS(app, resources={r"/api/*": {"origins": "*"}})

    # --- 5. Registrar Rutas (Blueprints) ---
    # (En una app más grande, moverías las rutas a otros archivos)
    # Por ahora, las dejamos aquí, pero deben estar DENTRO de la factory
    # o adjuntadas al 'app' global (lo haremos simple por ahora).
...
# --- RUTAS DE LA API ---
# (Todo tu código de rutas va aquí, SIN CAMBIOS)
# (Solo asegúrate de que 'app.route' use la 'app' global que creamos)

# Ruta de prueba de conexion a la db
@app.route('/api/test')
def hello_world():
    try:
        # Usamos el contexto de la app para asegurar la conexión
...
def crear_reserva():
    ...
    try:
        ...
        # (NUEVO) Guardamos el session_id de Stripe en la reserva
        success_url_template = "http://localhost:3000/pago-exitoso?session_id={CHECKOUT_SESSION_ID}"
        ...
        # (NUEVO) Guardamos el ID de sesión de Stripe en nuestra BD
...
def stripe_webhook():
    ...
    if event['type'] == 'checkout.session.completed':
        ...
        if codigo_reserva:
            try:
                # Usamos el contexto de la app para operaciones de BD
                with app.app_context():
                    reserva = Reservas.query.filter_by(codigo_reserva=codigo_reserva).first()
...
@jwt_required()
def validar_ticket():
    # (El código de @jwt_required() protege esta ruta)
    data = request.get_json()
...
# (NUEVO) --- ENDPOINT: CONSULTAR ESTADO DE RESERVA ---
# El frontend llamará a esto después de ser redirigido por Stripe
@app.route('/api/estado-reserva-por-session', methods=['GET'])
def get_estado_reserva():
    session_id = request.args.get('session_id')
    if not session_id:
    ...
    try:
        # Buscamos la reserva usando la ID de la sesión de Stripe
        reserva = Reservas.query.filter_by(stripe_session_id=session_id).first()
        
        if not reserva:
            return jsonify({'error': 'Reserva no encontrada para esa sesión'}), 404
            
        # Devolvemos el estado actual de la BD
        return jsonify({
            'estado_pago': reserva.estado_pago,
            'codigo_reserva': reserva.codigo_reserva
        })
